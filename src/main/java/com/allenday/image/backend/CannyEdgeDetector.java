...
class CannyEdgeDetector {
     ...
     * <p>
     * returns a Gaussian kernel radius in pixels, must exceed 0.1f.
     */
    ...
    private void computeGradients(float kernelRadius, int kernelWidth) {
        ...
        for (int x = initX; x < maxX; x++) {
            for (int y = initY; y < maxY; y += width) {
                ...
                double orient;
                double min = 360;
                int[] direction = {0, 23, 45, 68, 90, 113, 135, 158, 180, -180, -158, -135, -113, -90, -68, -45, -23};
                /*
                ...
                if (xGrad * yGrad <= (float) 0 /*(1)*/
                    ...
                    magnitude[index] = gradMag >= MAGNITUDE_LIMIT ? MAGNITUDE_MAX : (int) (MAGNITUDE_SCALE * gradMag);
                    orient = 57.2957795 * Math.atan2(yGrad, xGrad);
                    //System.err.println(orient);

                    orientation[index] = 0;
                    for (int i : direction) {
                        double delta = Math.abs(orient - i);
                        if (delta < min) {
                            orientation[index] = i;
                            min = delta;
                        }
                    }
                    if (orientation[index] >= 180)
                        orientation[index] -= 180;
    ...
    private void thresholdEdges() {
        for (int i = 0; i < picsize; i++) {
            if (data[i] > 0) {
//				System.err.println("a\t" + data[i] + "\t" + orientation[i]);
                ...
                } else if (orientation[i] == 23) {
                    data[i] = 0xffff8800; //orange
                } else if (orientation[i] == 45) {
                    data[i] = 0xffffff00; //yellow
                } else if (orientation[i] == 68) {
                    data[i] = 0xff00ff00; //green
                } else if (orientation[i] == 90) {
                    data[i] = 0xff00ffff; //cyan
                } else if (orientation[i] == 113) {
                    data[i] = 0xff0000ff; //blue
                } else if (orientation[i] == 135) {
                    data[i] = 0xff0088ff; //violet
                } else if (orientation[i] == 158) {
                    data[i] = 0xffff00ff; //magenta
                } else {
...
//				data[i] = -1;				
                ...
                data[i] = 0xff000000; //black
//				System.err.println("b " + data[i]);
            }

//			data[i] = data[i] > 0 ? -1 : 0xff000000;

//			data[i] = data[i] > 0 ? 0x00000000 : 0xffffffff; //black/white alpha=1
        }
    }
